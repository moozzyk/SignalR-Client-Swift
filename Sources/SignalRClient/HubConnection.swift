...
public class HubConnection {
    ...
    convenience public init(connection: Connection, hubProtocol: HubProtocol, logger: Logger = NullLogger()) {
        self.init(
            connection: connection, hubProtocol: hubProtocol, hubConnectionOptions: HubConnectionOptions(),
            logger: logger)
    }
    ...
    public func on(method: String, callback: @escaping (_ argumentExtractor: ArgumentExtractor) throws -> Void) {
        ...
        if callbackRegistered {
            logger.log(
                logLevel: .warning,
                message: "Client side hub method '\(method)' was already registered and was overwritten")
        }
    }
    ...
    public func send(method: String, arguments: [Encodable], sendDidComplete: @escaping (_ error: Error?) -> Void) {
        ...
        guard ensureConnectionStarted(errorHandler: { sendDidComplete($0) }) else {
            logger.log(
                logLevel: .warning,
                message: "Sending to server side hub method '\(method)' failed, connection not started")
            return
        }
        ...
        do {
            ...
            logger.log(
                logLevel: .error, message: "Sending to server side hub method '\(method)' failed. Error: \(error)")
            self.callbackQueue.async {
                sendDidComplete(error)
            }
        }
    ...
    public func invoke(
        ...
        invoke(
            method: method, arguments: arguments, resultType: DecodableVoid.self,
    ...
    public func invoke<T: Decodable>(
        method: String, arguments: [Encodable], resultType: T.Type,
    ...
    public func stream<T: Decodable>(
        method: String, arguments: [Encodable], streamItemReceived: @escaping (_ item: T) -> Void,
    ...
    public func cancelStreamInvocation(streamHandle: StreamHandle, cancelDidFail: @escaping (_ error: Error) -> Void) {
        ...
        do {
            ...
            connection.send(
                data: cancelInvocationData,
                sendDidComplete: { error in
                    if let e = error {
                        self.logger.log(
                            logLevel: .error,
                            message: "Sending cancellation of server side streaming hub returned error: \(e)")
                        self.callbackQueue.async {
                            cancelDidFail(e)
                        }
                    } else {
            ...
            logger.log(
                logLevel: .error, message: "Sending cancellation of server side streaming hub method failed: \(error)")
            self.callbackQueue.async {
                cancelDidFail(error)
            }
        }
    ...
    fileprivate func invoke(invocationHandler: ServerInvocationHandler) -> String {
        ...
        do {
            ...
            connection.send(data: invocationData) { error in
                if let e = error {
                    self.logger.log(
                        logLevel: .error, message: "Invoking server hub method \(method) returned error: \(e)")
    ...
    private func ensureConnectionStarted(errorHandler: @escaping (Error) -> Void) -> Bool {
        guard handshakeStatus.isHandled else {
            ...
            callbackQueue.async {
                errorHandler(
                    SignalRError.invalidOperation(
                        message: "Attempting to send data before connection has been started."))
            }
            return false
        }
        return true
    ...
    fileprivate func connectionDidReceiveData(data: Data) {
        ...
        if !handshakeStatus.isHandled {
            logger.log(
                logLevel: .debug,
                message: "Processing handshake response: \(String(data: data, encoding: .utf8) ?? "(invalid)")")
        ...
        do {
            ...
            for incomingMessage in messages {
                ...
                case MessageType.Close:
                    connection.stop(
                        stopError: SignalRError.serverClose(message: (incomingMessage as! CloseMessage).error))
    ...
    private func handleStreamItem(message: StreamItemMessage) throws {
        ...
        if serverInvocationHandler != nil {
            if let error = serverInvocationHandler!.processStreamItem(streamItemMessage: message) {
                ...
                failInvocationWithError(
                    invocationHandler: serverInvocationHandler!, invocationId: message.invocationId, error: error)
            }
        } else {
            logger.log(logLevel: .error, message: "Could not find callback with id \(message.invocationId)")
        }
    ...
    private func sendKeepAlivePing() {
        ...
        do {
            ...
            connection.send(
                data: cachedPingMessage,
                sendDidComplete: { error in
                    if let error = error {
                        self.logger.log(
                            logLevel: .error, message: "Keep alive send error:  \(error.localizedDescription)")
                    } else {
...
private class HubConnectionConnectionDelegate: ConnectionDelegate {
